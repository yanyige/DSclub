    ...
    window.onscroll = function () {
        if (checkScorllTop()) {
            ...
            for (var i = 0; i < dataInt.data.length; i++) {
                ...
                img.setAttribute('src', 'images/' + dataInt.data[i].src);
    ...
    function waterfall() {
        ...
        for (var i = 0; i < boxs.length; i++) {
            if (i < cols) {
                ...
                boxs[i].style.top = 375 + minHeight + "px";
                boxs[i].style.left = boxs[index].offsetLeft + "px";
                harr[index] += boxs[i].offsetHeight;
            }
        }
        allHeight = Math.min.apply(null , harr);
        wrap.style.cssText = "width:" + bWidth * cols + "px;margin: 0 auto;" + "height:" + allHeight + "px;";
    }
